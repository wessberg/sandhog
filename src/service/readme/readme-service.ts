...
export class ReadmeService implements IReadmeService {
	...
	private introHeader ({packageJson, license}: IReadmeServiceHeaderOptions): string {
		return `
			...
			${this.generateImage({height: this.config.readme.badgeHeight, alt: "Downloads per month", url: `https://npmcharts.com/compare/${packageJson.name}?minimal=true`, imageUrl: `https://img.shields.io/npm/dm/${encodeURIComponent(packageJson.name)}.svg`})}
			${this.generateImage({height: this.config.readme.badgeHeight, alt: "Dependencies", url: `https://david-dm.org/${this.stripLeadingScope(packageJson.name)}`, imageUrl: `https://img.shields.io/david/${this.stripLeadingScope(packageJson.name)}.svg`})}
			${this.generateImage({height: this.config.readme.badgeHeight, alt: "NPM Version", url: `https://www.npmjs.com/package/${packageJson.name}`, imageUrl: `https://badge.fury.io/js/${encodeURIComponent(packageJson.name)}.svg`})}
