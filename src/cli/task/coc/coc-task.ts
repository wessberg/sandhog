...
export async function cocTask({pkg, logger, prettier, config, root, fs, yes}: CocTaskOptions): Promise<void> {
	...
	const path = join(root, CONSTANT.CODE_OF_CONDUCT_FILENAME);
	...
	const writePermission =
		...
		(await confirm(`A ${CONSTANT.CODE_OF_CONDUCT_FILENAME} file already exists at path: ${path}. Do you wish to overwrite it?`, false));
